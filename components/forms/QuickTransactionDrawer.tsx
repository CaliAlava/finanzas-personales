...
export function QuickTransactionDrawer({
  ...
  // Categorías de ingresos extras
  const incomeCategories = categories.filter((c) => c.type === 'income');

  // Inicializar categoría activa
  React.useEffect(() => {
    if (mode === 'variable' && !categoryId && variableCategories.length > 0) {
      setCategoryId(variableCategories[0].id);
    } else if (mode === 'extra_income' && (!categoryId || !incomeCategories.some(c => c.id === categoryId))) {
      setCategoryId(incomeCategories[0]?.id || '');
    } else if (mode === 'goal_deposit' && !goalId && goals.length > 0) {
      setGoalId(defaultGoalId || goals[0].id);
    }
  }, [mode, variableCategories, incomeCategories, goals, defaultGoalId, categoryId, goalId]);
